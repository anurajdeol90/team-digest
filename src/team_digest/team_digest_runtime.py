...
def aggregate_range(
    ...
    *,
    ...
    for d in daterange(start, end):
        ...
        if secs.get("Decisions"):
            ...
            decisions_count += (
                len(dec_bul)
                if dec_bul
                else sum(1 for L in secs["Decisions"].splitlines() if L.strip())
            )
        if secs.get("Risks"):
            ...
            risks_count += (
                len(risk_bul)
                if risk_bul
                else sum(1 for L in secs["Risks"].splitlines() if L.strip())
            )
        ...
        if not bullets and secs.get("Actions"):
            ...
            if any(
                re.search(r"\[(?:high|medium|low|p0|p1|p2)\]", L, re.I) for L in lines
            ):
                bullets = lines
        for line in bullets:
    ...
    title = title or (
        f"Team Digest ({start.isoformat()} - {end.isoformat()})"
        if start != end
        else f"Team Digest ({start.isoformat()})"
    )
    out = [f"# {title}", ""]
    out.append(
    ...
    if emit_kpis:
        ...
        out.append(
            f"- **Actions:** {len(action_items)} (High: {H}, Medium: {M}, Low: {L})"
        )
        out.append(f"- **Decisions:** {decisions_count}   ·   **Risks:** {risks_count}")
        out.append(
            f"- **Owners:** {owners_total}   ·   **Days with notes:** {len(matched)}"
        )
        out.append("")
        if owner_breakdown and owner_counts:
            ...
            rest = rows[owner_top:]
            if rest:
                rh = sum(r[1] for r in rest)
                rm = sum(r[2] for r in rest)
                rl = sum(r[3] for r in rest)
                rt = sum(r[4] for r in rest)
