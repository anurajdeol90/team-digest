...
def resolve_examples_logs():
    ...
    # Keep ExitStack alive by attaching to function attribute
...
def main():
    ...
    # Not fatal if they differ, but warn
    if dist_ver != mod_ver:
        print(f"WARNING: version drift (dist={dist_ver}, module={mod_ver})", file=sys.stderr)

    # 2) Ensure packaged examples exist
    exlogs = resolve_examples_logs()
    print("Packaged examples logs path:", exlogs)
    if not os.path.isdir(exlogs):
    ...
    # Weekly
    run(f'team-digest weekly --logs-dir "{exlogs}" --start 2025-10-13 --end 2025-10-19 '
        f'--output "{weekly_out}" --group-actions --emit-kpis --owner-breakdown')

    # Daily
    run(f'team-digest daily --logs-dir "{exlogs}" --date 2025-10-17 '
        f'--output "{daily_out}" --group-actions')

    # Monthly (month-to-date)
    run(f'team-digest monthly --logs-dir "{exlogs}" '
        f'--output "{monthly_out}" --group-actions --emit-kpis --owner-breakdown')

    # 4) Basic content assertions (not empty, has a section header)
