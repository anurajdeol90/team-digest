...
# Tolerant: accept ## or ###, any case, allow spaces; CRLF-safe
def read_sections(text):
    secs = {}
    text = text.replace("\r\n", "\n")
    for sec in SECTION_ORDER:
        pat = re.compile(
            rf"^[ \t]*###?\s*{re.escape(sec)}\s*$([\s\S]*?)(?=^[ \t]*###?\s+\S|\Z)",
            re.M | re.I,
        )
        m = pat.search(text)
        if m:
...
def main():
    ...
    out = [f"# {title}", ""]
    out.append(f"_Range: {start.isoformat()} → {end.isoformat()} | Source: {a.logs_dir} | Days matched: {len(matched)} | Actions: {len(action_items)}_")
    out.append("")
