...
def cmd_monthly(a):
    ...
    if a.year and a.month:
        ...
        end = (start.replace(day=1) + dt.timedelta(days=32)).replace(
            day=1
        ) - dt.timedelta(days=1)
    else:
...
def main(argv=None):
    ...
    def common(p):
        ...
        p.add_argument(
            "--post-to-slack",
            action="store_true",
            help="Post the rendered markdown to Slack via SLACK_WEBHOOK_URL",
        )
    ...
    common(p1)
    p1.add_argument(
        "--date", default=dt.datetime.utcnow().date().isoformat(), help="YYYY-MM-DD"
    )
    ...
    # monthly
    p3 = sub.add_parser(
        "monthly", help="Build a digest for a calendar month (or month-to-date)"
    )
    common(p3)
